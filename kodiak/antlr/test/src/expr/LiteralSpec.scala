...
class Literal extends ParserSpec:
  test("True") {
    assertParse(
      input = "true",
      expected = Ast.Program(Ast.True),
    )
  }
  test("False") {
    assertParse(
      input = "false",
      expected = Ast.Program(Ast.False),
    )
  }
  test("Unit") {
    assertParse(
      input = "unit",
      expected = Ast.Program(Ast.Unit),
    )
  }
  test("Simple integer") {
    assertParse(
      input = "42",
      expected = Ast.Program(Ast.Integer(42)),
    )
  }
  test("Simple decimal") {
    assertParse(
      input = "42.31",
      expected = Ast.Program(Ast.Decimal(42.31)),
    )
  }
  test("Text word") {
    assertParse(
      input = "\"Hello",
      expected = Ast.Program(Ast.PlainText("Hello")),
    )
  }
  test("Raw number word") {
    assertParse(
      input = "abc#123",
      expected = Ast.Program(Ast.RawNumber(Ast.Id("abc"), "123")),
    )
  }
  test("Tuple group") {
    assertParse(
  ...
  test("Seq with one item") {
    assertParse(
      input = "[42,]",
      expected = Ast.Program(Ast.Seq(Ast.Integer(42))),
    )
  }
  test("Seq with two items") {
    assertParse(
      input = "[42,43]",
      expected = Ast.Program(Ast.Seq(Ast.Integer(42), Ast.Integer(43))),
    )
  }
