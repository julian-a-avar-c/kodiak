...
trait Exprs extends Core with Types:
  ...
  object StatCtx
      extends WsCtx(semiInference = true, arrowTypeAscriptions = false)
  // Expressions nested within other expressions
  object ExprCtx
      extends WsCtx(semiInference = false, arrowTypeAscriptions = true)
  // Expressions directly within a `val x = ...` or `def x = ...`
  object FreeCtx
      extends WsCtx(semiInference = true, arrowTypeAscriptions = true)

  def TypeExpr[$: P] = ExprCtx.Expr

  class WsCtx(semiInference: Boolean, arrowTypeAscriptions: Boolean):

    def OneSemiMax[$: P] = if semiInference then OneNLMax else Pass
    def NoSemis[$: P]    = if semiInference then NotNewline else Pass
    ...
    def Expr[$: P]: P[Unit] =
      ...
      def DoWhile = P(
        `do` ~/ Expr ~ Semis.? ~ `while` ~ "(" ~ ExprCtx.Expr ~ ")",
      )
      ...
      def LambdaRhs = if semiInference then P(BlockChunk) else P(Expr)

      def ImplicitLambda = P(
        `implicit` ~ (Id | Underscore) ~ (`:` ~ InfixType).? ~ `=>` ~ LambdaRhs.?,
      )
      def ParenedLambda = P(
        Parened ~~ (WL ~ `=>` ~ LambdaRhs.? | ExprSuffix ~~ PostfixSuffix ~ SuperPostfixSuffix),
      )
      def PostfixLambda = P(
        PostfixExpr ~ (`=>` ~ LambdaRhs.? | SuperPostfixSuffix).?,
      )
    ...
    def AscriptionType[$: P]     =
      if arrowTypeAscriptions then P(Type) else P(InfixType)
    def Ascription[$: P] = P(
      `:` ~/ (`Underscore*` | AscriptionType | Annot.rep(1)),
    )
    def MatchAscriptionSuffix[$: P] = P(
      `match` ~/ "{" ~ CaseClauses | Ascription,
    )
    def ExprPrefix[$: P] = P(WL ~ CharIn("\\-+!~") ~~ !syntax.Basic.OpChar ~ WS)
    def ExprSuffix[$: P] = P(
      (WL ~ "." ~/ Id | WL ~ TypeArgs | NoSemis ~ ArgList).repX ~~ (NoSemis ~ Underscore).?,
    )
    ...
    def PostfixExpr[$: P]: P[Unit] = P(
      PrefixExpr ~~ ExprSuffix ~~ PostfixSuffix,
    )
    ...
    def SimpleExpr[$: P]: P[Unit] =
      ...
      P(
        New | BlockExpr | ExprLiteral | StableId | Underscore | Parened,
      )
    end SimpleExpr
  ...
  def SimplePattern[$: P]: P[Unit] =
    ...
    def Thingy    = P(
      Underscore ~ (`:` ~/ TypePat).? ~ !("*" ~~ !syntax.Basic.OpChar),
    )
  ...
  def BlockLambdaHead[$: P]: P[Unit] = P(
    "(" ~ BlockLambdaHead ~ ")" | `this` | Id | Underscore,
  )
  ...
  def BaseBlock[$: P](end: => P[Unit])(implicit
  ...
  def BindPattern[$: P]: P[Unit]       =
    def InfixPattern = P(
      SimplePattern ~ (Id ~/ SimplePattern).rep | `Underscore*`,
    )
  ...
  def ParenArgList[$: P] = P(
    "(" ~/ (Exprs ~ (`:` ~/ `Underscore*`).?).? ~ TrailingComma ~ ")",
  )
  ...
  def CaseClauses[$: P]: P[Unit] =
    ...
    def CaseClause: P[Unit] = P(
      `case` ~ !(`class` | `object`) ~/ Pattern ~ ExprCtx.Guard.? ~ `=>` ~ CaseBlock,
    )
    P(CaseClause.rep(1) ~ "}")
  end CaseClauses
end Exprs
