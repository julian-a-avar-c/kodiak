...
object Identifiers:
  ...
  def PlainId[$: P]                      = P(
    UppercaseId(true) | VarId | Operator ~ (!OpChar | &(StringIn("/*", "//"))),
  )
    .opaque("plain-id")

  def PlainIdNoDollar[$: P] =
    P(UppercaseId(false) | VarId0(false) | Operator).opaque("plain-id")
  ...
  def IdRest[$: P](allowDollar: Boolean) =
    ...
    val IdCharacter =
      if allowDollar then
        NamedFunction(c => c == '$' || isLetter(c) || isDigit(c))
