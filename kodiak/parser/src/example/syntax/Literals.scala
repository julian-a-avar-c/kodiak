...
trait Literals:
  ...
  def Semis[$: P]: P[Unit] = P(
    NoTrace(NoCut(WS) ~ Basic.Semi.rep(1, NoCut(WS)) ~ NoCut(WS)),
  )
  def Newline[$: P]: P[Unit] = P(WL ~ Basic.Newline)

  def NotNewline[$: P]: P[Unit] = P(&(WS ~ !Basic.Newline))
  def OneNLMax[$: P]: P[Unit]   =
    def ConsumeComments = P(
      (Basic.WSChars.? ~ NoTrace(
        Literals.Comment,
      ) ~ Basic.WSChars.? ~ Basic.Newline).rep,
    )
  ...
  object Literals:
    ...
    def Float[$: P] =
      ...
      def FloatSuffix     = P(
        LeadingDotFloat | Exp ~ FloatType.? | Exp.? ~ FloatType,
      )
    ...
    class InterpCtx(interp: Option[() => P[Unit]]):
      ...
      /** Helper to quickly gobble up large chunks of un-interesting characters.
        * We break out conservatively, even if we don't know it's a "real"
        * escape sequence: worst come to worst it turns out to be a dud and we
        * go back into a CharsChunk next rep
        */
      def StringChars[$: P] = P(
        CharsWhile(c => c != '\n' && c != '"' && c != '\\' && c != '$'),
      )
      def NonTripleQuoteChar[$: P] = P(
        "\"" ~ "\"".? ~ !"\"" | CharIn("\\\\$\n"),
      )
      ...
      def SingleChars[$: P](allowSlash: Boolean) =
        def LiteralSlash = P(if allowSlash then "\\" else Fail)
