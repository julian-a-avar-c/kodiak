...
trait Core extends syntax.Literals:
  class TrailingCommaOps[+T](p0: => P[T]):
    def repTC[R](min: Int = 0, max: Int = Int.MaxValue, exactly: Int = -1)(using
        ev: fastparse.Implicits.Repeater[T, R],
        ctx: P[?],
    ): P[R] =
      p0.rep(min = min, sep = ",", max = max, exactly = exactly) ~ TrailingComma
  end TrailingCommaOps
  given TrailingCommaOps: [T] => (p0: P[T]) => TrailingCommaOps[T] =
    new TrailingCommaOps(p0)
...
//   def `<-`[$: P] = O("<-") | O("←").opaque("\"<-\"")
  ...
  /** Sketchy way to whitelist a few suffixes that come after a . select; apart
    ...
    */
  def PostDotCheck[$: P]: P[Unit] = P(
    WL ~ !(`super` | `this` | "{" | Underscore | `type`),
  )
